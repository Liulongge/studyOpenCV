...
int main()
{
    Mat origin_img = imread("1.jpg", IMREAD_GRAYSCALE);       //读入图像灰度图
    ...
    int m = getOptimalDFTSize(origin_img.rows);
    int n = getOptimalDFTSize(origin_img.cols);

    //填充输入图像I，输入矩阵为padded，上方和左方不做填充处理
    copyMakeBorder(origin_img, padded, 0, m - origin_img.rows, 0, n - origin_img.cols, BORDER_CONSTANT, Scalar::all(0));

    imshow("paded_img", origin_img);
    ...
    //=> log(1 + sqrt(Re(DFT(origin_img))^2 + Im(DFT(origin_img))^2))
    split(complexI, planes);        //planes[0] = Re(DFT(origin_img),planes[1] = Im(DFT(origin_img))
